...
class FM(BaseEstimator, TransformerMixin):
    def __init__(self, features_M, features_dim, pretrain_flag, save_file, hidden_factor, loss_type, epoch, batch_size, learning_rate, lamda_bilinear, keep,
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():  # , tf.device('/cpu:0'):
            ...
            bi = self.permutate(nonzero_embeddings)
            ...
            self.out = tf.add_n([bi, self.Feature_bias, Bias])  # None * 1
    ...
    def permutate(self, embeddings):
        v_perm = []
        for i in range(self.features_dim-1):
            for j in range(i+1,self.features_dim):
                v_i = embeddings[:,i,:]
                v_j = embeddings[:,j,:]
                #dot product of two vectors
                v_dotpro = tf.reduce_sum(tf.multiply(v_i, v_j), axis=1)
                v_perm.append(tf.expand_dims(v_dotpro, 1))
            
        v_concat = tf.concat(v_perm, axis=1) 
        return tf.reduce_sum(v_concat, axis=1, keep_dims=True)
...
if __name__ == '__main__':
    ...
    # Training
    t1 = time()
    model = FM(data.features_M, data.feature_dim, args.pretrain, save_file, args.hidden_factor, args.loss_type, args.epoch, args.batch_size, args.lr, args.lamda, args.keep_prob, args.optimizer, args.batch_norm, args.verbose)
